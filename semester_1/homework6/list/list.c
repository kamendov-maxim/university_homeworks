...
static Node *mergeSortedCompare(Node *first, Node *second, bool byName)
{
    Node *temp = NULL;
    ...
    if (byName)
    {
        ...
        {
            temp = second;
            temp->next = mergeSortedCompare(second->next, first, byName);
        }
        else
        {
            temp = first;
            temp->next = mergeSortedCompare(second, first->next, byName);
        }
    }
    else
    {
...
static void mergeSortRecursion(Node **head, bool byName)
{
    ...
    *head = mergeSortedCompare(first, second, byName);
}
